...
## 테스트
### 1) H2 인메모리 db를 활용한 테스트
* 환경설정 : `src/test/resources/application.yml`
* 테스트 데이터 구성 : `src/test/resources/schema.sql`, `src/test/resources/data.sql`

#### 동시성 테스트 시나리오 및 결과
1. 선착순 30명 신청 테스트
* 목적: 40명이 동시에 신청할 때 정확히 30명만 신청되는지 검증
* 결과:
    * 성공: 정확히 30명 신청 완료
    * 실패: 10명이 정원 초과로 실패
    * 최종 인원이 정확히 30명으로 유지됨

2. 마지막 한 자리 경쟁 테스트
* 목적: 현재 인원 29명인 상태에서 마지막 1자리에 대한 동시 신청 처리 검증
* 결과:
    * 10명 중 1명만 성공
    * 나머지 9명은 정원 초과로 실패
    * 최종 인원이 정확히 30명으로 유지됨
...
3. 동일 사용자 중복 신청 테스트
* 목적: 같은 사용자가 동시에 여러 번 신청할 때 중복 신청 방지 검증
* 결과:
    * 5번의 시도 중 1번만 성공
    * 나머지 4번은 중복 신청으로 실패
    * 데이터베이스의 유니크 제약조건이 정상 작동

#### 결론

    * 동시성 제어가 정상적으로 작동하여 정원 관리가 정확히 이루어짐
    * 중복 신청 방지 메커니즘이 효과적으로 작동
    * 트랜잭션 처리와 락 메커니즘이 안정적으로 동작

### 2) MySql db를 활용한 E2E 테스트
* 데이터베이스: MySQL
* API 테스트 도구: Postman
...
#### API 테스트 시나리오 및 결과
1. 특강 신청 API
* REGISTRATION 테이블에 신규 레코드 생성 확인
* SCHEDULE 테이블의 CURRENT_COUNT 증가 확인
* 응답 데이터 정상 수신
  
2. 특강 목록 조회 API
* LECTURE 테이블의 데이터와 응답 데이터 일치 확인
* 각 강의별 스케줄 정보가 SCHEDULE 테이블과 일치

3. 신청한 특강 조회 API
*  REGISTRATION 테이블의 데이터와 응답 일치 확인
*  사용자별 신청 이력 정상 조회

4. 데이터베이스 정합성 검증
* REGISTRATION 테이블
    * 신청 시 REGISTRATION_ID 자동 증가 확인
    * (SCHEDULE_ID, USER_ID) 유니크 제약조건 정상 동작
    * CREATED_AT 타임스탬프 정상 기록
* SCHEDULE 테이블
    * CURRENT_COUNT 업데이트 정확성 확인
    * CAPACITY 제한 로직 정상 동작
* LECTURE 테이블
    * 강의 기본 정보 정상 유지
    * 조회 시 관련 스케줄 정보와 정상 연결

#### 결론
* 세 개의 주요 API가 실제 운영 환경과 동일한 조건에서 정상 동작
* 테이블 간 관계 및 제약조건이 설계대로 작동
* 실제 사용자 시나리오에 따른 E2E 흐름이 원활히 진행됨
